defmodule PasswordGenerator do
  @moduledoc """
  Generates random passwords depending on parameters, Module main function is generate`(options)`.
  ...
  @spec generate(option :: map()) :: {:ok, bitstring()} | {:error, bitstring()}
   ...
   defp included_options(options) do
     Enum.filter(options, fn {_key, value} -> value |> String.trim() |> String.to_existing_atom()
   end)
